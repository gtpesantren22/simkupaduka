...
Terimakasih';
	...
	public function pengajuanDtl($kode)
	{
		...
		if ($data['rinci']->cair == 1) {
			$data['data'] = $this->db->query("SELECT realis.*, rab.rencana, rab_sm24.kegiatan FROM realis JOIN rab ON realis.kode=rab.kode JOIN rab_sm24 ON realis.kode=rab_sm24.kode WHERE realis.kode_pengajuan = '$kode' ")->result();
			...
			$data['data'] = $this->db->query("SELECT real_sm.*, rab.rencana, rab_sm24.kegiatan FROM real_sm JOIN rab ON real_sm.kode=rab.kode JOIN rab_sm24 ON real_sm.kode=rab_sm24.kode WHERE real_sm.kode_pengajuan = '$kode' ")->result();
