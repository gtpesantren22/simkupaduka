	...
	public function process_uploadDppk()
	{
		...
		if (!$this->upload->do_upload('uploadFile')) {
			...
			for ($row = 2; $row <= $highestRow; $row++) {
				...
				$kegiatan = mb_convert_encoding(preg_replace('/[^\x20-\x7E]/', '', addslashes($worksheet->getCell('C' . $row)->getValue())), 'UTF-8', 'auto');
				$tahun = mb_convert_encoding(preg_replace('/[^\x20-\x7E]/', '', addslashes($worksheet->getCell('D' . $row)->getValue())), 'UTF-8', 'auto');
