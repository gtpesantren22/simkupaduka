	...
	public function process_uploadDppk()
	{
		...
		if (!$this->upload->do_upload('uploadFile')) {
			...
			for ($row = 2; $row <= $highestRow; $row++) {
				...
				$kegiatan = trim($this->db->escape(preg_replace('/[^\x20-\x7E]/', '', addslashes($worksheet->getCell('C' . $row)->getValue()))), "'");
				$tahun = trim($this->db->escape(preg_replace('/[^\x20-\x7E]/', '', addslashes($worksheet->getCell('D' . $row)->getValue()))), "'");
